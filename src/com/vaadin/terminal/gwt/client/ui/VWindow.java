...
public class VWindow extends VOverlay implements Container,
    ...
    /**
     * fix for #5495, #5497 taken from GWT issue
    ...
    protected void onPreviewNativeEvent(NativePreviewEvent event) {
        ...
        if (ne.getKeyCode() == KeyCodes.KEY_TAB) {
            ...
            if (Element.is(target) == false
                    || getElement().isOrHasChild(Element.as(target)) == false) {
                focus();
                /*
                 * -- proposed solution from google groups if (ne.getShiftKey()
                 * == false) { // Set focus to the first field } else { // Set
                 * focus to the last field }
                 */
            }
        }
